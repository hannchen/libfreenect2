...
/** @file dump_and_cpudepth_packet_processor.cpp Depth processor implementation for the CPU. */
...
class DumpAndCpuDepthPacketProcessorImpl: public WithPerfLogging
{
public:
  ...
  Frame *ir_frame, *depth_frame, *inactive_frame;

  bool flip_ptables;

  DumpAndCpuDepthPacketProcessorImpl()
  {
    newIrFrame();
    newDepthFrame();
    newInactiveFrame();
  ...
  void newInactiveFrame()
  {
    inactive_frame = new Frame(512, 424, 4);
    inactive_frame->format = Frame::Float;
  }
                          ...
                          float *inactive_out)
  {
    ...
    *inactive_out = (float) decodePixelMeasurement(data, 9, x, y);
...
DumpAndCpuDepthPacketProcessor::DumpAndCpuDepthPacketProcessor() :
    impl_(new DumpAndCpuDepthPacketProcessorImpl())
{
}
...
void DumpAndCpuDepthPacketProcessor::setConfiguration(const libfreenect2::DepthPacketProcessor::Config &config)
{
 ...
 */
void DumpAndCpuDepthPacketProcessor::loadP0TablesFromCommandResponse(unsigned char* buffer, size_t buffer_length)
{
...
void DumpAndCpuDepthPacketProcessor::loadXZTables(const float *xtable, const float *ztable)
{
...
void DumpAndCpuDepthPacketProcessor::loadLookupTable(const short *lut)
{
 ...
 */
void DumpAndCpuDepthPacketProcessor::process(const DepthPacket &packet)
{
  ...
  impl_->inactive_frame->timestamp = packet.timestamp;
  impl_->ir_frame->sequence = packet.sequence;
  impl_->depth_frame->sequence = packet.sequence;
  impl_->inactive_frame->sequence = packet.sequence;

  Mat<Vec<float, 9> >
      m(424, 512),
      m_filtered(424, 512)
  ;
  Mat<float> inactive(424, 512, impl_->inactive_frame->data);
  ...
  for(int y = 0; y < 424; ++y)
    ...
    {
        impl_->processPixelStage1(x, y, packet.buffer, m_ptr + 0, m_ptr + 3, m_ptr + 6, &inactive.at(y, x));
    }
